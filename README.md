...
> 六阶段闭环中最关键的六个设计。**点击展开** 查看每一项「解决什么问题 + 怎么做」。

<details>
<summary><b>1️⃣ 自维护历史 + 带键置顶记忆</b> —— 破解「框架不回传历史」的失忆问题</summary>

<br/>

Agent 在进程内自管两类上下文，**只存文本摘要、不存历史截图**，严格控制 token：

| 记忆类型 | 存储形式 | 机制 |
|---|---|---|
| **动作历史** | `List[{step, action, params, note}]` | 渲染**最近 20 步**（`HISTORY_WINDOW=20`），每条附精简 Thought 作观察备注，供上下文与防重复 |
| **带键置顶记忆** | 有序字典 `store_key → (展示键, 值)` | 📌 `REMEMBER: 值` 固定事实去重长留；🔄 `REMEMBER【键】: 值` 过程信息**同键覆盖** |

- 容量上限 `16 条 × 500 字符`，渲染为 `## Key Info To Remember (pinned)` **永久钉在 Prompt 顶部**，不随历史窗口滚走。
- **持久化规划**：长任务首步用 `REMEMBER【计划】` 钉住有序计划，`REMEMBER【进度】` 同键覆盖刷新，**跨几十步不丢线**。

</details>

<details>
<summary><b>2️⃣ 11 种动作的稳健解析</b> —— 保证零非法动作中断　<code>utils/finals_decoder.py</code></summary>
...
```text
剥离 <think> → 取最后一条 Action: → 函数式 name(args)
   → 冒号式 CLICK:[[x,y]] → 宽松兜底提取坐标 → 仍失败则兜底 COMPLETE
```

- 坐标兼容 `<point>x y</point>` / `[x,y]` / `x,y` / `x y`，统一钳制到 `[0,1000]`。
- `double_click` 先于 `click` 匹配（避免子串误判）；`<enter>` / `回车` 归一为换行符。

</details>

<details>
<summary><b>3️⃣ 通用 Prompt 与策略体系</b> —— 零任务硬编码的「操作手册」　<code>utils/finals_prompt.py</code></summary>

<br/>

**完整动作空间 + 坐标系说明 + 1 条置顶 OPEN 纪律 + 23 条编号策略**，全部为可泛化软技能。摘录：

- **OPEN 纪律**：启动任何 App 一律 `open(app_name='中文全名')`，绝不在桌面找图标。
- **搜索流程**：识别灰色占位符 → 聚焦 → 输入 → 确认 → 提交。
- **电商**：属性无选项先看图、**无货换品不假完成**、订单停收银页不真支付。
- **收集 N 个**：逐项进详情看属性、`REMEMBER` 记录已加 / 已拒。

</details>

<details>
<summary><b>4️⃣ 轻量通用护栏</b> —— 兜底纠错而不过度干预　<code>agent.py</code></summary>
...
<details>
<summary><b>5️⃣ 界面停滞反思 + 卡死强制脱困</b> —— 检测画面是否真的变了</summary>

<br/>

对相邻帧做 **32×32 灰度平均哈希（1024 位）** 算汉明距离（阈值 10），细网格能识别「展开下拉 / 勾选筛选」等小幅变化。**强制接管只保留「绝对安全」的死局**：

- **强制 BACK（仅 2 类）**：① 连续 ≥6 步「画面不变且原地戳同一处」；② 连续 5 步纯 WAIT 且画面冻结。
- **强制 SCROLL**：动作循环 / 连戳同一小块 → 探索滚动打破。

> 早期「横跳 BACK / 反复点击软提示」会干扰正常重试，已移除，收敛到上述绝对安全场景。

</details>

<details>
<summary><b>6️⃣ 按需 App 专属提示</b> —— 针对性提示而不污染通用性　<code>APP_HINTS</code></summary>

<br/>

针对「因入口隐蔽而易失败」的常用 App 维护极简提示表，**仅命中时注入一条**（不撑 token）。命中方式：指令显式名优先 + 意图关键词兜底（如「导航 / 路线」→ 高德地图）。
