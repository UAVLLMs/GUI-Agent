...
> 本项目在 **第十六届「中兴捧月」全球精英挑战赛总决赛** 中荣获 **全国第二名**，
> 并获得 **中兴校招 SSP 面试直通卡** 与 **5 万元奖金**。
...
<sub>▶ 真机实时操控 · 自动完成「高德地图找附近评分最高咖啡馆并导航」全流程 
...
### 🧠 自维护历史 + 带键置顶记忆
针对"框架不回传历史"，Agent 内部自管两类上下文：**动作历史**（最近 20 步）+ **REMEMBER 置顶记忆**（固定事实长期保留、过程信息同键覆盖），并据此持久化跨几十步的计划与进度。
...
- **动作历史** `List[{step, action, parameters, note}]`：每步成功后追加，渲染**最近 20 步**（`HISTORY_WINDOW=20`）注入 Prompt，提供上下文与防重复依据。每条附带该步**精简 Thought** 作为"观察备注"。**只存文本摘要、不存历史截图**，严格控制 token。
- **带键置顶记忆（REMEMBER 机制）**：用有序字典 `store_key → (展示键, 值)` 存储，支持两种写法：
  - 📌 **固定事实** `REMEMBER: 值` —— 按内容去重、写一次长期保留（如榜单前五歌名）。
  - 🔄 **过程信息** `REMEMBER【键】: 值` —— **同键覆盖**最新值，避免旧快照堆积矛盾（如计划 / 进度 / 已加清单）。
  - 容量上限 `16 条 × 500 字符`，渲染为 `## Key Info To Remember (pinned)` **永久钉在 Prompt 顶部**，不随历史窗口滚走。
- **持久化规划**：对"≥4 子目标"的长任务，引导模型首步用 `REMEMBER【计划】` 钉住有序计划，并用 `REMEMBER【进度】`（同键覆盖）随完成刷新，**跨几十步不丢线**。
- `reset()` 在每个用例开始清空，杜绝跨用例污染。

### 4.2 11 种动作的稳健解析 `（utils/finals_decoder.py）`

「从严到松」多级解析链，保证**零非法动作中断**：
...
### 4.3 通用 Prompt 与策略体系 `（utils/finals_prompt.py）`
...
### 4.5 界面停滞反思 + 卡死强制脱困 `（ReAct 中的 reflection）`
...
### 4.6 按需 App 专属提示 `（APP_HINTS）`

针对"确因找不到入口而易失败"的常用 App，维护一张极简提示表，**仅命中时注入一条**（不污染通用性、不撑 token）。命中方式：指令显式名优先 + 意图关键词兜底（如"导航 / 路线"→ 高德地图）。
...
| `SCROLL` | `start_point` + `end_point` | 滚动 |
...
覆盖**电商、视频、音频、地图出行、音乐、学习资讯及跨 App 协作**等典型场景，所有用例 `agent_completed`，且**实际步数均显著低于步数上限**，体现高效与稳健。

### 案例一 · 搜索播放与内容互动
> *"在 QQ 音乐搜索光年之外并播放邓紫棋演唱的版本" / "哔哩哔哩搜索 Python 入门教程播放播放量最高的"*

![搜索播放与内容互动](assets/montage_main_1.png)

### 案例二 · 多重筛选与出行预订
> *"携程预订 6/28-6/30 丽江低价优先最便宜的单人女生床位房" / "滴滴查途经点询价"*

![多重筛选与出行预订](assets/montage_main_2.png)

### 案例三 · 复杂收集与跨 App 协作
> *"得到搜索 skill 课程收集 4 门评分>4.8 加入书单" / "**豆瓣音乐榜前五 → QQ 音乐建歌单并播放**"（跨 App 长任务）*

![复杂收集与跨App协作](assets/montage_main_3.png)

### 案例四 · 多样高难交互动作
> 覆盖 `DOUBLE_CLICK` 双击点赞、`LONG_PRESS` 长按菜单、`DRAG` 拖动进度条 + 倍速
...
- **鲁棒性**：3 次重试 + 线性退避；所有输出经标准化兜底，保证必过框架动作校验，**结构上不存在无限挂死**。
